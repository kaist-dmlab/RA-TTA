...
from src.dataloader.templates import imagenet
...
parser.add_argument('--qthres', type=float, default=0.90)
...
templates = imagenet
if dataset in ['imagenet_v2', 'imagenet_1k', 'imagenet_sketch']:
    aux_embedding_path = os.path.join(args.db_root, f'{database}_imagenet_1k_aux_{_net_name}')
    with open(f'{aux_embedding_path}', 'rb') as f:
        file = pkl.load(f)
        aux_embeds = file['embedding'].astype('float16')

    aux_size = aux_embeds.shape
    aux_index = np.random.randint(aux_size[0], size=int(aux_size[0] * 0.75)) # due to VRAM size
    aux_embeds = aux_embeds[aux_index]
else:
...
if args.dataset in ['imagenet_1k', 'imagenet_v2', 'imagenet_adversarial']:
    pre_template = "a photo of the {}. "
elif args.dataset == 'imagenet_rendition':
    pre_template = "a rendition photo of the {}. "
elif args.dataset == 'imagenet_sketch':
    pre_template = "a sketch photo of a {}. "
